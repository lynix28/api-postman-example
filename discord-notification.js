...
const discordNotification = new DiscordNotification.DiscordNotification('API-Postman-Example', process.env.DISCORD_WEBHOOK);
...
function test(state) {
	if (state == 'start') {
		discordNotification
			...
			.addField({name: 'Build Number', value: `api.v.1.${process.env.BUILD}` }) 
